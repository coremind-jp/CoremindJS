...
&&  $_POST["oldFile"])
{
    $new = unescape($_POST["newFile"]);
    echo $new;
    $old = unescape($_POST["oldFile"]);
    echo $old;
}



function splitBlock($source)
{
    $classDefPos = strpos("$deine:", $source);
    
    
    ereg_replace("\{[\n|.]*\}", $joinedSource, "");
}
function splitByClassBlock($source)
{
    return preg_split("cm.Class.create(\n?", $source);
}
function getImportClasses($source) {
    return preg_split(",", substr($source, 0, strpos("{", $source)));
}
function getExtensClass($source) {
   // return $source, strpos("{", $source), strpos("{", $source));
}
